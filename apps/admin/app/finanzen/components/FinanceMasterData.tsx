...
interface RateRow { _id:string; employeeName?:string; funktion?:string; validFrom:string; baseHourlyCents:number; travelHourlyCents:number; nightSurchargePercent:number; sundaySurchargePercent:number; holidaySurchargePercent:number }
...
export function FinanceMasterData({ data, onChanged }: { data: FinanceOverviewDto; onChanged: () => void }) {
  ...
  const [rate, setRate] = useState({ employeeId:'', funktion:'', validFrom:dateInput(), base:'', travel:'', night:'25', sunday:'50', holiday:'100' })
  ...
  const submitRate = () => run(async () => {
    const response = await fetchWithIntent('/api/finanzen/employee-rates', { method:'POST', intent:'finance:rate:create', body:JSON.stringify({
      employeeId:rate.employeeId, funktion:rate.funktion, validFrom:rate.validFrom, baseHourlyCents:Math.round(Number(rate.base.replace(',','.')) * 100),
  ...
  return <div className="space-y-4">
    ...
    <Dialog open={rateOpen} onOpenChange={setRateOpen}><DialogContent><DialogHeader><DialogTitle>Lohnsatz hinzufügen</DialogTitle><DialogDescription>Historische Sätze bleiben unverändert erhalten.</DialogDescription></DialogHeader><div className="grid gap-4 sm:grid-cols-2"><Field label="Mitarbeiter"><Select value={rate.employeeId} onValueChange={employeeId=>setRate({...rate,employeeId})}><SelectTrigger><SelectValue placeholder="Mitarbeiter wählen"/></SelectTrigger><SelectContent>{employees.map(item=><SelectItem key={item._id} value={item._id}>{item.name}{item.miNumber ? ` · ${item.miNumber}` : ''}</SelectItem>)}</SelectContent></Select></Field><Field label="Funktion"><Select value={rate.funktion} onValueChange={funktion=>setRate({...rate,funktion})}><SelectTrigger><SelectValue placeholder="Funktion wählen"/></SelectTrigger><SelectContent>{MITARBEITER_FUNKTION_OPTIONS.map(item=><SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}</SelectContent></Select></Field><Field label="Gültig ab"><Input type="date" value={rate.validFrom} onChange={event=>setRate({...rate,validFrom:event.target.value})}/></Field><Field label="Grundlohn (€ / h)"><Input type="number" step="0.01" value={rate.base} onChange={event=>setRate({...rate,base:event.target.value})}/></Field><Field label="Fahrt (€ / h)"><Input type="number" step="0.01" value={rate.travel} onChange={event=>setRate({...rate,travel:event.target.value})}/></Field><Field label="Nacht (%)"><Input type="number" value={rate.night} onChange={event=>setRate({...rate,night:event.target.value})}/></Field><Field label="Sonntag (%)"><Input type="number" value={rate.sunday} onChange={event=>setRate({...rate,sunday:event.target.value})}/></Field><Field label="Feiertag (%)"><Input type="number" value={rate.holiday} onChange={event=>setRate({...rate,holiday:event.target.value})}/></Field></div><DialogFooter><Button variant="outline" onClick={()=>setRateOpen(false)}>Abbrechen</Button><Button onClick={submitRate} disabled={busy || !rate.employeeId || !rate.funktion || !rate.base}>{busy && <Loader2 className="mr-2 h-4 w-4 animate-spin"/>}Satz speichern</Button></DialogFooter></DialogContent></Dialog>
  </div>
}
