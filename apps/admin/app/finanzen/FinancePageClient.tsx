...
interface FinancePeriod { from: string; to: string }

export default function FinancePageClient() {
  // null = noch kein Zeitraum gewählt: der erste Aufruf lädt den gesamten Datenbestand,
  // der Server meldet den tatsächlich belegten Zeitraum zurück.
  ...
  const load = useCallback(async () => {
    ...
    try { const params=new URLSearchParams();if(period){params.set('from',period.from);params.set('to',period.to)}if(projectId)params.set('projectId',projectId);if(accountId)params.set('accountId',accountId);const response=await fetch(`/api/finanzen/overview?${params}`,{credentials:'include'});const result=await response.json();if(!response.ok)throw new Error(result.error||'Finanzdaten konnten nicht geladen werden.');const overview=result.data as FinanceOverviewDto;setData(overview);if(!period)setPeriod({from:dateInput(overview.period.from),to:dateInput(overview.period.to)}) } catch(cause){setError(cause instanceof Error?cause.message:'Finanzdaten konnten nicht geladen werden.')} finally{setLoading(false)}
  },[accountId,period,projectId])
