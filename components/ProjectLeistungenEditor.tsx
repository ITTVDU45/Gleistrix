              ...
              <div key={pos.id} className="rounded-md border border-slate-100 bg-slate-50 p-2 dark:border-slate-700 dark:bg-slate-800/50">
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    placeholder="Position (z. B. 02.01.0010)"
                    className="w-40"
                  />
                  <Input
                  ...
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    className="h-9 shrink-0 text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20"
                ...
                <div className="mt-2 grid grid-cols-2 gap-2 md:grid-cols-4">
                  <Input value={pos.menge || ''} onChange={(e) => updatePos(pi, xi, { menge: e.target.value })} placeholder="Menge" />
                  <Input value={pos.einheit || ''} onChange={(e) => updatePos(pi, xi, { einheit: e.target.value })} placeholder="Einheit (Stück/Meter/Tage)" />
                  <Input value={pos.einzelpreis || ''} onChange={(e) => updatePos(pi, xi, { einzelpreis: e.target.value })} placeholder="Einzelpreis" />
                  <Input value={pos.gesamtsumme || ''} onChange={(e) => updatePos(pi, xi, { gesamtsumme: e.target.value })} placeholder="Gesamtsumme" />
                </div>
              </div>
            ))}
