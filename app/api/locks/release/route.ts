...
export async function POST(req: NextRequest) {
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      logger.debug('[API] Sperre-Freigabe-Request erhalten', { resourceType, resourceId, user: currentUserName })
    }
    ...
    if (process.env.NODE_ENV === 'development') {
      logger.debug('=== SPERRE FREIGEBEN ANFORDERUNG ===')
      logger.debug(`Ressource: ${resourceType}/${resourceId}`)
      logger.debug(`Benutzer: ${currentUserName ?? 'unbekannt'}`)
    ...
    if (existingLocks.length === 0) {
      if (process.env.NODE_ENV === 'development') {
        logger.debug('[API] Keine Sperre gefunden zum Freigeben', { resourceType, resourceId })
        logger.debug('===========================')
      }
    ...
    if (process.env.NODE_ENV === 'development') {
      logger.debug('[API] Sperre erfolgreich freigegeben, sende WebSocket-Update', { deletedCount: result.deletedCount })
      logger.debug(`Freigegebene Sperren: ${result.deletedCount} fuer ${resourceType}/${resourceId}`)
      logger.debug('===========================')
    }
    ...
    try {
      ...
      logger.debug('[API] WebSocket nicht verfuegbar fuer Lock-Update')
    }
