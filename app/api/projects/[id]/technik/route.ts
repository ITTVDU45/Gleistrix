...
export async function POST(
  ...
  try {
    ...
    logger.debug('Technik POST Request:', { id, date, technik });
    ...
    logger.debug('Vor dem Speichern - Projekt Technik:', {
    ...
    logger.debug('Saving project with technik:', {
    ...
    if (currentUser) {
      try {
        ...
        logger.debug('Activity Log erstellt für Technik-Hinzufügung');
      } catch (logError) {
    ...
    logger.debug('Nach dem Speichern - Gespeicherte Technik:', {
...
export async function PUT(
  ...
  try {
    ...
    logger.debug('Technik PUT Request Body:', parseResult.data);
    logger.debug('Technik PUT Request Parsed:', { id, date, technikId, updatedTechnik, selectedDays });

    // Verbesserte Validierung
    if (!date) {
      logger.error('Fehlende date in Request');
      return NextResponse.json(
    ...
    if (!technikId) {
      logger.error('Fehlende technikId in Request');
      return NextResponse.json(
    ...
    if (!updatedTechnik) {
      logger.error('Fehlende updatedTechnik in Request');
      return NextResponse.json(
    ...
    if (!updatedTechnik.name || !updatedTechnik.anzahl || updatedTechnik.meterlaenge === undefined) {
      logger.error('Ungültige updatedTechnik Felder:', updatedTechnik);
      return NextResponse.json(
    ...
    const project = await Project.findById(id);
    if (!project) {
      logger.error('Projekt nicht gefunden:', id);
      return NextResponse.json(
        { error: 'Projekt nicht gefunden' },
        { status: 404 }
      );
    ...
    logger.debug('Projekt gefunden, Technik vor Bearbeitung:', {
      hasTechnik: !!project.technik,
      technikKeys: project.technik ? Object.keys(project.technik) : []
    });
    ...
    if (selectedDays && Array.isArray(selectedDays) && selectedDays.length > 0) {
      logger.debug('Bearbeite Technik für mehrere Tage:', selectedDays);
      ...
      for (const selectedDate of selectedDays) {
        ...
        if (idx !== -1) {
          ...
          logger.debug(`Technik aktualisiert für ${selectedDate}:`, currentTechnik[idx]);
        } else {
          ...
          logger.debug(`Neue Technik erstellt für ${selectedDate}:`, newTechnik);
        }
      ...
      logger.debug('Bearbeite Technik für einzelnen Tag:', date);
      
      if (!project.technik || !project.technik[date]) {
        logger.error('Kein Technik-Eintrag für Datum:', date);
        return NextResponse.json(
      ...
      const idx = currentTechnik.findIndex((t: any) => t.id === technikId);
      if (idx === -1) {
        logger.error('Technik-Eintrag nicht gefunden für ID:', technikId);
        return NextResponse.json(
      ...
      logger.debug('Technik aktualisiert für einzelnen Tag:', currentTechnik[idx]);
    }
    ...
    logger.debug('Projekt nach Bearbeitung:', {
      atwsImEinsatz: project.atwsImEinsatz,
      anzahlAtws: project.anzahlAtws,
      gesamtMeterlaenge: project.gesamtMeterlaenge,
      technikKeys: Object.keys(project.technik)
...
export async function DELETE(
  ...
  try {
    ...
    logger.debug('Technik DELETE Request:', { id, date, technikId });
    ...
    logger.error('Fehler beim Löschen der Technik:', error);
    return NextResponse.json(
      { error: 'Serverfehler', details: error },
      { status: 500 }
    );
