...
export async function POST(
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'production' && csrf !== 'lager:article:image:presign') {
    ...
    const objectKey = getArticleImageObjectKey(id, `${attachmentId}-${body.filename}`)

    const expires = 60 * 10
    let uploadUrl =
      typeof (minioClient as any).presignedPutObjectAsync === 'function'
        ? await (minioClient as any).presignedPutObjectAsync(bucket, objectKey, expires)
        : await new Promise<string>((resolve, reject) =>
            (minioClient as any).presignedPutObject(bucket, objectKey, expires, (err: unknown, url: string) =>
    ...
    console.error('Presign-Artikelbild-Upload fehlgeschlagen:', error)
    return NextResponse.json(
      { success: false, message: 'Upload-Vorbereitung fehlgeschlagen' },
      { status: 500 }
    )
  }
}
